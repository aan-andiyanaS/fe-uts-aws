...
export default function Register() {
  ...
  // state notifikasi di tengah layar
    ...
    try {
      ...
      if (res.ok) {
        setNotif({
          ...
          message: data.msg || "Akun kamu sudah dibuat. Silakan login.",
        });

        // kasih waktu user lihat notif dulu
        setTimeout(() => {
        ...
        setNotif({
          ...
          message:
            data.msg ||
            "Pastikan nama, email, dan password sudah diisi dengan benar.",
        });
      }
    } catch (error) {
      console.error(error);
      ...
      <div className="relative w-full max-w-xl">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-slate-300">
            Buat akun baru untuk mulai kelola produk di TOHE.
          </p>
        </div>
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full rounded-xl bg-gradient-to-r from-indigo-500 via-purple-500 to-sky-500 py-2.5 text-sm font-semibold text-white shadow-[0_14px_45px_rgba(129,140,248,0.55)] transition transform ${
              loading
                ? "opacity-70 cursor-not-allowed"
                : "hover:shadow-[0_18px_60px_rgba(129,140,248,0.8)] hover:-translate-y-[1px]"
            }`}
          >
